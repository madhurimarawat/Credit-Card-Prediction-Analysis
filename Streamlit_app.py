...
data_name = "Credit Card Approval Prediction"
...
Updated_train_data = pd.read_csv("Updated_Train_Data.csv")

Updated_test_data = pd.read_csv("Updated_Test_Data.csv")
...
x_train, y_train = (
    Updated_train_data.drop(columns=["Is high risk"], axis=1),
    Updated_train_data["Is high risk"],
)

x_test, y_test = (
    Updated_test_data.drop(columns=["Is high risk"], axis=1),
    Updated_test_data["Is high risk"],
)
...
def info(data_name):
    ...
    st.write("Shape of Training Dataset is: ", Updated_train_data.shape)
    st.write("Shape of Testing Dataset is: ", Updated_test_data.shape)
    st.write("Number of classes: ", len(np.unique(Updated_train_data["Is high risk"])))

    # Making a dataframe to store target name and value
    df = pd.DataFrame(
        {
            "Target Value": list(np.unique(Updated_train_data["Is high risk"])),
            "Target Name": ["No Risk", "High Risk"],
        }
    )
...
display_background_image(
    "https://thumbs.dreamstime.com/b/credit-score-vector-illustration-loan-arrow-gauge-speedometer-indicator-poor-to-good-rate-flat-cartoon-hand-drawn-279410151.jpg",
    0.8,
)
